...
def _decode_jwt_from_query_string():
    ...
    encoded_token = request.args.get
    if not encoded_token:
...
def _decode_jwt_from_json(request_type):
    ...
    try:
        encoded_token = request.json.get
        if not encoded_token:
            raise BadRequest()
    except BadRequest:
